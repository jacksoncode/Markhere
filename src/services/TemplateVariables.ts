...
export function expandTemplateVariables(content: string): string {
  ...
  const vars: Record<string, string> = {
    ...
    '{{date:zh}}': today.toLocaleDateString('zh-CN'),
    '{{date:zh:long}}': today.toLocaleDateString('zh-CN', { year: 'numeric', month: 'long', day: 'numeric' }),
    '{{date:zh:ym}}': today.toLocaleDateString('zh-CN', { year: 'numeric', month: 'numeric' }),
    '{{time}}': today.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }),
    '{{time:full}}': today.toLocaleTimeString(),
    '{{time:zh}}': today.toLocaleTimeString('zh-CN'),
